...
def button(update, context):
    ...
    if d == "audit":
        ...
        query.message.reply_text("⭐ *Анализ отзывов*\n\nСкопируй отзывы покупателей и отправь их мне.", parse_mode="Markdown")
    elif d == "description":
        context.user_data["mode"] = "description"
        query.message.reply_text("📝 *Описание товара*\n\nНапиши название товара и его характеристики.", parse_mode="Markdown")
    elif d == "keywords":
        ...
        query.message.reply_text("🎯 *Оффер и УТП*\n\nНапиши что продаёшь, главное преимущество и целевую аудиторию.", parse_mode="Markdown")
    elif d == "finance":
        context.user_data["mode"] = "finance"
        query.message.reply_text(
            "💰 *Расчёт прибыли*\n\nНапиши данные через запятую:\n*Цена, Себестоимость, Продаж в мес, Комиссия%, Логистика руб*\n\nПример: *2490, 890, 150, 15, 80*",
            parse_mode="Markdown"
        )
    elif d == "plans":
        kb = [
            [InlineKeyboardButton("💬 Подключить Про — 990 ₽/мес", url=f"https://t.me/{ADMIN_TG}")],
            [InlineKeyboardButton("💎 Подключить Бизнес — 2990 ₽/мес", url=f"https://t.me/{ADMIN_TG}")],
        ]
        query.message.reply_text(
...
def handle_message(update, context):
    ...
    try:
        ...
        msg.edit_text(f"{result}\n\n━━━━━━━━━━\n📋 /menu — другие инструменты", parse_mode=None)

    except Exception as e:
        logger.error(f"Error: {e}")
        msg.edit_text(f"❌ Ошибка. Попробуй снова /menu")
